  ...
  <body style="background-color: rgb(207, 207, 207)">
    <div id="inicio" class="container-fluid">
      <header>
        ...
        <nav>
          <div class="navbar">
          <a class="ingreso backButton" href="balance.php"
...
<div
      ...
      <form action="<?=$_SERVER['PHP_SELF']?>" method="post">
        <div class="searchPartner">
          ...
          <div>
            <input href="movimientos.php" type="submit" class="ingreso2" value="MOSTRAR TODOS">
          </div>
        </div>
      </form>
        <div class="tabla text-center">
            <table class="table">
              ...
              <?php
                if(isset($_POST['enviar'])){
                  ...
                  if(empty($_POST['fecha'])){
                    echo '<script language="javascript"> alert ("Debe ingersar una fecha para poder buscarlo!!!"); window.location.href="ingresos.php" </script>';
                  }else{
                    if(isset($_POST['fecha'])){
                        $fecha = $_POST['fecha'];
                        // Convertir la fecha al formato aaaa/mm/dd
                       
                      $sql="SELECT * FROM ingresos where mes = STR_TO_DATE('".$fecha."', '%d-%m-%Y')";
                    }
                  }
                  $resultado = mysqli_query($conectar, $sql);
              ...
              ?>
                  <tr>
                  <td data-label="Numero de Socio"><?php echo $filas['mes'] ?></td>
                    <td data-label="Nombre y Apellido">$<?php echo $filas['cuota'] ?></td>
                    <td data-label="DNI">$<?php echo $filas['deporte'] ?></td>
